...
export default function CreateCurriculumPage() {
  ...
  useEffect(() => {
    if (prodiId) {
      const fetchProdi = async () => {
        const { data } = await supabase.from('prodi').select('nama').eq('id', prodiId).single();
        if (data) {
          setProdiName(data.nama);
        }
      };
      fetchProdi();
    }
  ...
  const handleSave = async () => {
    ...
    if (error) {
      ...
      alert("Kurikulum berhasil dibuat");
      router.push("/admin/curriculum/foundation");
    }
  };
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-gray-500">
          Program Studi: {prodiName || "Memuat..."}
        </p>
      </div>
