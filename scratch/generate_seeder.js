...
const lines = [];
lines.push("-- ============================================================================");
lines.push("-- SEEDER DATA CPMK PRODI KEBIDANAN");
lines.push("-- Menambahkan Master Prodi Kebidanan, Kurikulum Kebidanan, CPL, Bahan Kajian,");
lines.push("-- Mata Kuliah, serta 92 CPMK beserta Kedalaman KAP (Kognitif, Afektif, Psikomotorik).");
lines.push("-- ============================================================================");
lines.push("");
lines.push("DO $$");
lines.push("DECLARE");
lines.push("  v_prodi_id uuid;");
lines.push("  v_kurikulum_id uuid;");
lines.push("  v_jurusan_id uuid;");
lines.push("  v_mk_id uuid;");
lines.push("  v_bk_id uuid;");
lines.push("BEGIN");
lines.push("  -- 1. Dapatkan atau Buat Prodi Kebidanan");
lines.push("  SELECT id INTO v_prodi_id FROM prodi WHERE nama ILIKE '%Kebidanan%' LIMIT 1;");
lines.push("  IF v_prodi_id IS NULL THEN");
lines.push("    SELECT id INTO v_jurusan_id FROM jurusan LIMIT 1;");
lines.push("    IF v_jurusan_id IS NULL THEN");
lines.push("      v_jurusan_id := uuid_generate_v4();");
lines.push("      INSERT INTO jurusan (id, nama) VALUES (v_jurusan_id, 'Fakultas Ilmu Kesehatan');");
lines.push("    END IF;");
lines.push("    v_prodi_id := uuid_generate_v4();");
lines.push("    INSERT INTO prodi (id, nama, jurusan_id) VALUES (v_prodi_id, 'Kebidanan', v_jurusan_id);");
lines.push("  END IF;");
lines.push("");
lines.push("  -- 2. Dapatkan atau Buat Kurikulum Kebidanan");
lines.push("  SELECT id INTO v_kurikulum_id FROM kurikulum WHERE prodi_id = v_prodi_id LIMIT 1;");
lines.push("  IF v_kurikulum_id IS NULL THEN");
lines.push("    v_kurikulum_id := uuid_generate_v4();");
lines.push("    INSERT INTO kurikulum (id, prodi_id, nama, tahun_berlaku)");
lines.push("    VALUES (v_kurikulum_id, v_prodi_id, 'Kurikulum Kebidanan OBE 2024', 2024);");
lines.push("  END IF;");
lines.push("");

// Unique CPLs and BKs
const uniqueCpls = [...new Set(rawJson.map(i => i.cpl.trim()))];
const uniqueBks = [...new Set(rawJson.flatMap(i => i.kode_bk.split(',').map(b => b.trim())))];

lines.push("  -- 3. Inisialisasi CPL Kebidanan");
uniqueCpls.forEach(cpl => {
  lines.push(`  IF NOT EXISTS (SELECT 1 FROM cpl WHERE kurikulum_id = v_kurikulum_id AND kode = '${cpl}') THEN`);
  lines.push(`    INSERT INTO cpl (id, kurikulum_id, kode, deskripsi) VALUES (uuid_generate_v4(), v_kurikulum_id, '${cpl}', '${cpl} Kebidanan');`);
  lines.push(`  END IF;`);
});
lines.push("");

lines.push("  -- 4. Inisialisasi Bahan Kajian (BK) Kebidanan");
uniqueBks.sort((a,b) => parseNum(a) - parseNum(b)).forEach(bk => {
  lines.push(`  IF NOT EXISTS (SELECT 1 FROM bahan_kajian WHERE kurikulum_id = v_kurikulum_id AND kode = '${bk}') THEN`);
  lines.push(`    INSERT INTO bahan_kajian (id, kurikulum_id, kode, nama) VALUES (uuid_generate_v4(), v_kurikulum_id, '${bk}', 'Bahan Kajian ${bk}');`);
  lines.push(`  END IF;`);
});
lines.push("");

lines.push("  -- 5. Insert CPMK dan Pemetaan ke Mata Kuliah & BK");

// Calculate counts per MK for bobot
const mkCounts = {};
rawJson.forEach(item => {
  mkCounts[item.mata_kuliah] = (mkCounts[item.mata_kuliah] || 0) + 1;
});

rawJson.forEach(item => {
  const mkNama = item.mata_kuliah.replace(/'/g, "''");
  const cpmkKode = item.kode_cpmk.replace(/'/g, "''");
  const cpmkDesk = item.deskripsi_cpmk.replace(/'/g, "''");
  const kVal = parseNum(item.kedalaman_k);
  const aVal = parseNum(item.kedalaman_a);
  const pVal = parseNum(item.kedalaman_p);
  
  const bks = item.kode_bk.split(',').map(b => b.trim());
  const bkFirst = bks[0];
  const count = mkCounts[item.mata_kuliah];
  const bobot = (100.0 / count).toFixed(2);
  
  lines.push(`  -- MK: ${mkNama} | CPMK: ${cpmkKode}`);
  lines.push(`  SELECT id INTO v_mk_id FROM mata_kuliah WHERE kurikulum_id = v_kurikulum_id AND nama = '${mkNama}' LIMIT 1;`);
  lines.push(`  IF v_mk_id IS NULL THEN`);
  lines.push(`    v_mk_id := uuid_generate_v4();`);
  lines.push(`    INSERT INTO mata_kuliah (id, kurikulum_id, kode, nama, sks, semester, sifat_mk)`);
  lines.push(`    VALUES (v_mk_id, v_kurikulum_id, 'MK-${String(item.no).padStart(2, '0')}', '${mkNama}', 3, 1, 'Wajib');`);
  lines.push(`  END IF;`);
  
  bks.forEach(bkSingle => {
    lines.push(`  INSERT INTO mata_kuliah_bk (mata_kuliah_id, bk_id)`);
    lines.push(`  SELECT v_mk_id, id FROM bahan_kajian WHERE kurikulum_id = v_kurikulum_id AND kode = '${bkSingle}'`);
    lines.push(`  ON CONFLICT DO NOTHING;`);
  });

  lines.push(`  SELECT id INTO v_bk_id FROM bahan_kajian WHERE kurikulum_id = v_kurikulum_id AND kode = '${bkFirst}' LIMIT 1;`);
  lines.push(`  INSERT INTO cpmk (id, kode, deskripsi, bobot, kedalaman_k, kedalaman_a, kedalaman_p, mata_kuliah_id, bk_id)`);
  lines.push(`  VALUES (uuid_generate_v4(), '${cpmkKode}', '${cpmkDesk}', ${bobot}, ${kVal}, ${aVal}, ${pVal}, v_mk_id, v_bk_id);`);
  lines.push(``);
});

lines.push("END $$;");

fs.writeFileSync("seeder_cpmk_kebidanan.sql", lines.join("\n"), "utf8");
console.log("seeder_cpmk_kebidanan.sql generated successfully!");
